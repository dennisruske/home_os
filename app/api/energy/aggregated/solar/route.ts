...
function aggregateByHour(
  readings: Array<{ timestamp: number; solar: number }>
): AggregatedDataPoint[] {
  ...
  const hourBuckets = new Map<number, Array<{ timestamp: number; solar: number }>>();

  for (const reading of readings) {
    // Only process positive solar production
    if (reading.solar <= 0) {
      continue;
    }
  ...
  for (const [hourTimestamp, bucketReadings] of hourBuckets.entries()) {
    ...
    for (let i = 0; i < bucketReadings.length - 1; i++) {
      const power1 = Math.max(0, bucketReadings[i].solar); // watts, ensure positive
      const power2 = Math.max(0, bucketReadings[i + 1].solar); // watts, ensure positive
...
function aggregateByDay(
  ...
  const dayBuckets = new Map<number, Array<{ timestamp: number; solar: number }>>();
...
function calculateTotalEnergy(
  ...
  // Filter for positive solar production only
  const positiveReadings = readings.filter((r) => r.solar > 0);
  ...
  for (let i = 0; i < sortedReadings.length - 1; i++) {
    const power1 = Math.max(0, sortedReadings[i].solar); // watts, ensure positive
    const power2 = Math.max(0, sortedReadings[i + 1].solar); // watts, ensure positive
